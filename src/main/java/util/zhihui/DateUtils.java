 ...
 * @(#)DateUtils.java
 *
 * Copyright 2012 sohu tv. All rights reserved.
 */
package util.zhihui;
...
public class DateUtils {
    ...
    /** 日期格式 */
    public static final String DEFAULT_DATEHOUR_FORMAT = "yyyy-MM-dd HH";
    ...
    public static Date getDateHour(String date) {
        return getDate(date, DEFAULT_DATEHOUR_FORMAT, null);
    }
    ...
    public static String formateDateHour(Date date) {
        return formatDate(date, DEFAULT_DATEHOUR_FORMAT, null);
    }
    ...
    public static Date plusMonth(Date date, int monthes) {
        if (date == null) {
            date = new Date();
        }
        java.util.Calendar c = java.util.Calendar.getInstance();
        c.setTime(date);
        c.add(java.util.Calendar.MONTH, monthes);// 加一个月
        return c.getTime();
    }
